 ...
 * Copyright (C) 2000-2019 i-scream
 * Copyright (C) 2010-2019 Jens Rehsack
 * 
...
add_stat(stat_type type, void *stat, ...) {
	...
	if (display_mode == DISPLAY_GRAPHITE &&
	    ((type == STAT_TYPE_BOOL) || (type == STAT_TYPE_STRING) ||
	     (type == STAT_TYPE_DUPLEX))) {
		return;
	}
...
populate_const(void) {
	...
	add_stat(STAT_TYPE_INT, &zero, "const", "0", NULL);
}
...
populate_cpu(void) {
	...
	if (use_cpu_percent) {
		...
		if (cpu_p != NULL) {
			add_stat(STAT_TYPE_DOUBLE, &cpu_p->user,
				 "cpu", "user", NULL);
			add_stat(STAT_TYPE_DOUBLE, &cpu_p->kernel,
				 "cpu", "kernel", NULL);
			add_stat(STAT_TYPE_DOUBLE, &cpu_p->idle,
				 "cpu", "idle", NULL);
			add_stat(STAT_TYPE_DOUBLE, &cpu_p->iowait,
				 "cpu", "iowait", NULL);
			add_stat(STAT_TYPE_DOUBLE, &cpu_p->swap,
				 "cpu", "swap", NULL);
			add_stat(STAT_TYPE_DOUBLE, &cpu_p->nice,
				 "cpu", "nice", NULL);
			add_stat(STAT_TYPE_TIME_T, &cpu_p->time_taken,
				 "cpu", "time_taken", NULL);
		}
	} else {
		if (cpu_s != NULL) {
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->user,
				 "cpu", "user", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->kernel,
				 "cpu", "kernel", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->idle,
				 "cpu", "idle", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->iowait,
				 "cpu", "iowait", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->swap,
				 "cpu", "swap", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->nice,
				 "cpu", "nice", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->total,
				 "cpu", "total", NULL);
			add_stat(STAT_TYPE_TIME_T, &cpu_s->systime,
				 "cpu", "systime", NULL);
		}
	}

	if (cpu_s != NULL) {
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->context_switches,
			 "cpu", "ctxsw", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->voluntary_context_switches,
			 "cpu", "vctxsw", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->involuntary_context_switches,
			 "cpu", "nvctxsw", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->syscalls,
			 "cpu", "syscalls", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->interrupts,
			 "cpu", "intrs", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &cpu_s->soft_interrupts,
			 "cpu", "softintrs", NULL);
	}
}
...
populate_mem(void) {
	...
	if (mem != NULL) {
		add_stat(STAT_TYPE_BYTES, &mem->total, "mem", "total", NULL);
		add_stat(STAT_TYPE_BYTES, &mem->free, "mem", "free", NULL);
		add_stat(STAT_TYPE_BYTES, &mem->used, "mem", "used", NULL);
		add_stat(STAT_TYPE_BYTES, &mem->cache, "mem", "cache", NULL);
	}
}
...
populate_user(void) {
	...
	if (users != NULL) {
		...
		for (i = 0; i < entries; i++) {
			...
			add_stat(STAT_TYPE_STRING, &users[i].login_name,
				 "user", tty, "login_name", NULL);
			add_stat(STAT_TYPE_STRING, &users[i].device,
				 "user", tty, "tty", NULL);
			add_stat(STAT_TYPE_STRING, &users[i].hostname,
				 "user", tty, "from", NULL);
			add_stat(STAT_TYPE_TIME_T, &users[i].login_time,
		...
		add_stat(STAT_TYPE_INT, &entries, "user", "num", NULL);
		add_stat(STAT_TYPE_STRING, &name_list, "user", "names", NULL);
	}
}
...
populate_swap(void) {
	...
	if (swap != NULL) {
		add_stat(STAT_TYPE_BYTES, &swap->total, "swap", "total", NULL);
		add_stat(STAT_TYPE_BYTES, &swap->used, "swap", "used", NULL);
		add_stat(STAT_TYPE_BYTES, &swap->free, "swap", "free", NULL);
	}
}
...
populate_general(void) {
	...
	if (host != NULL) {
		add_stat(STAT_TYPE_STRING, &host->os_name,
			 "general", "os_name", NULL);
		add_stat(STAT_TYPE_STRING, &host->os_release,
			 "general", "os_release", NULL);
		add_stat(STAT_TYPE_STRING, &host->os_version,
			 "general", "os_version", NULL);
		add_stat(STAT_TYPE_STRING, &host->platform, "general", "platform", NULL);
		add_stat(STAT_TYPE_STRING, &host->hostname, "general", "hostname", NULL);
		add_stat(STAT_TYPE_UNSIGNED, &host->ncpus, "general", "ncpus", NULL);
		add_stat(STAT_TYPE_UNSIGNED, &host->maxcpus, "general", "ncpus_cfg", NULL);
		add_stat(STAT_TYPE_UNSIGNED, &host->bitwidth, "general", "bitwidth", NULL);
		add_stat(STAT_TYPE_STRING, ((size_t)host->host_state) > (lengthof(host_states) - 1)
		                 ? &unexpected_host_state
		                 : &host_states[host->host_state], "general", "hoststate", NULL);
		add_stat(STAT_TYPE_TIME_T, &host->uptime, "general", "uptime", NULL);
	}
}
...
populate_fs(void) {
	...
	if (disk != NULL) {
		for (i = 0; i < n; i++) {
			...
			add_stat(STAT_TYPE_STRING, &disk[i].device_name,
				 "fs", name, "device_name", NULL);
			add_stat(STAT_TYPE_STRING, &disk[i].device_canonical,
				 "fs", name, "device_canonical", NULL);
			add_stat(STAT_TYPE_STRING, &disk[i].fs_type,
				 "fs", name, "fs_type", NULL);
			add_stat(STAT_TYPE_STRING, &disk[i].mnt_point,
				 "fs", name, "mnt_point", NULL);
			add_stat(STAT_TYPE_BYTES, &disk[i].size,
				 "fs", name, "size", NULL);
			add_stat(STAT_TYPE_BYTES, &disk[i].used,
				 "fs", name, "used", NULL);
			add_stat(STAT_TYPE_BYTES, &disk[i].avail,
				 "fs", name, "avail", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].total_inodes,
				 "fs", name, "total_inodes", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].used_inodes,
				 "fs", name, "used_inodes", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].free_inodes,
				 "fs", name, "free_inodes", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].avail_inodes,
				 "fs", name, "avail_inodes", NULL);
			add_stat(STAT_TYPE_BYTES, &disk[i].io_size,
				 "fs", name, "io_size", NULL);
			add_stat(STAT_TYPE_BYTES, &disk[i].block_size,
				 "fs", name, "block_size", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].total_blocks,
				 "fs", name, "total_blocks", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].free_blocks,
				 "fs", name, "free_blocks", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].avail_blocks,
				 "fs", name, "avail_blocks", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &disk[i].used_blocks,
...
populate_disk(void) {
	...
	if (diskio != NULL) {
		for (i = 0; i < n; i++) {
			...
			add_stat(STAT_TYPE_STRING, &diskio[i].disk_name,
				 "disk", name, "disk_name", NULL);
			add_stat(STAT_TYPE_BYTES, &diskio[i].read_bytes,
				 "disk", name, "read_bytes", NULL);
			add_stat(STAT_TYPE_BYTES, &diskio[i].write_bytes,
				 "disk", name, "write_bytes", NULL);
			add_stat(STAT_TYPE_TIME_T, &diskio[i].systime,
...
populate_proc(void) {
	...
	if (proc != NULL) {
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &proc->total, "proc", "total", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &proc->running, "proc", "running", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &proc->sleeping, "proc", "sleeping", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &proc->stopped, "proc", "stopped", NULL);
		add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &proc->zombie, "proc", "zombie", NULL);
	}
	else {
		char *errbuf;
...
populate_net(void) {
	...
	if (io != NULL) {
		for (i = 0; i < num_io; i++) {
			...
			add_stat(STAT_TYPE_STRING, &io[i].interface_name,
				 "net", name, "interface_name", NULL);
			add_stat(STAT_TYPE_BYTES, &io[i].tx,
				 "net", name, "tx", NULL);
			add_stat(STAT_TYPE_BYTES, &io[i].rx,
				 "net", name, "rx", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &io[i].ipackets,
				 "net", name, "ipackets", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &io[i].opackets,
				 "net", name, "opackets", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &io[i].ierrors,
				 "net", name, "ierrors", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &io[i].oerrors,
				 "net", name, "oerrors", NULL);
			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &io[i].collisions,
				 "net", name, "collisions", NULL);
			add_stat(STAT_TYPE_TIME_T, &io[i].systime,
	...
	if (iface != NULL) {
		for (i = 0; i < num_iface; i++) {
			...
			if (!had_io) {
				add_stat(STAT_TYPE_STRING, &iface[i].interface_name,
					"net", name, "interface_name", NULL);
			}

			add_stat(STAT_TYPE_UNSIGNED_LONG_LONG, &iface[i].speed,
				 "net", name, "speed", NULL);
			add_stat(STAT_TYPE_BOOL, &iface[i].up,
				 "net", name, "up", NULL);
			add_stat(STAT_TYPE_DUPLEX, &iface[i].duplex,
...
#endif
		break;
	case STAT_TYPE_UNSIGNED_LONG_LONG:
